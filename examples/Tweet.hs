...
main = runBH' $ do
  ...
  let idxTpl = IndexTemplate (TemplatePattern "tweet-*") (Just (IndexSettings (ShardCount 1) (ReplicaCount 1) Nothing)) [toJSON TweetMapping]
  ...
  where
    ...
    indexSettings = IndexSettings (ShardCount 1) (ReplicaCount 0) Nothing
